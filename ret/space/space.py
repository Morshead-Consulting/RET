...
from ret.space.culturemap import CultureMap
from ret.space.feature import Area
from ret.space.terrain import Terrain
from ret.types import Coordinate2d, Coordinate3d  # noqa: TC001
from ret.utilities import geometric_utilities
...
if TYPE_CHECKING:
    ...
    from ret.space.feature import Boundary
    from ret.space.heightband import HeightBand
    from ret.types import Color, Coordinate, Coordinate2dOr3d, Vector2d
...
class ContinuousSpaceWithTerrainAndCulture2d(ContinuousSpace):
    ...
    def get_clockwise_heading_in_degrees_in_xy_plane(
        ...
        return geometric_utilities._heading_to_clockwise_angle_degrees(heading)
    ...
    def get_precipitation(self) -> Precipitation:
        ...
        Returns:
            Precipitation: The precipitation enum.
        """
...
class ContinuousSpaceWithTerrainAndCulture3d(ContinuousSpaceWithTerrainAndCulture2d):
    ...
    def place_agent(self, agent: RetAgent, pos: Coordinate2dOr3d) -> None:
        ...
        """
        super().place_agent(agent, self.get_coordinate_3d(pos, agent=agent))
    ...
    def get_neighbors(
        ...
        """
        deltas = self._agent_points - np.array(self.get_coordinate_3d(pos), dtype="int64")
        dists = deltas[:, 0] ** 2 + deltas[:, 1] ** 2 + deltas[:, 2] ** 2

        (idxs,) = np.where((dists <= radius**2) & (dists >= 0))
    ...
    def get_coordinate_3d(
        ...
        bands: list[HeightBand] = None,
        agent: Optional[RetAgent] = None,
    ) -> Coordinate3d:
        ...
        Args:
            ...
            bands (list[HeightBand], optional): Bands if needing to convert a
                Coordinate3dBand. Defaults to None.
            agent (Optional[RetAgent]): The agent having its coordinates converted.
                Defaults to None.
        ...
        if hes_len and len(in_coordinate) == 2:
            ...
            if agent is not None:
                warnings.warn(
                    "2D coordinate was entered in 3D terrain, "
                    + "placing coordinate at terrain level",
                    stacklevel=2,
                )
    ...
    def is_agent_above_terrain(self, agent: RetAgent) -> bool:
        ...
        pos_3d = self.get_coordinate_3d(agent.pos, agent=agent)
        z = pos_3d[2]
